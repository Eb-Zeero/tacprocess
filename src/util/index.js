  ...
  proposals.forEach(p => {
    [0, 1, 2, 3, 4].forEach( pr => {
      if(p.allocatedTime && p.allocatedTime[ partner ]){
        total[ `p${ pr }` ] += parseFloat(p.allocatedTime[ partner ] ? p.allocatedTime[ partner ][ `p${ pr }` ] : 0) ||
          0
      }
    })
  })
...
const pageRole = (page, role) => {
  ...
  if ((page === types.TECHNICAL_PAGE || page === types.LIAISON_PAGE) && (role === types.SALT_ASTRONOMER )) { return true }
